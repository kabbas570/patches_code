...
class Dataset_(Dataset):
    def __init__(self, image_dir, mask_dir,scar_dir):
        self.image_dir = image_dir
        self.mask_dir = mask_dir
        self.scar_dir=scar_dir
    ...
    def __getitem__(self, index):
        ...
        scar_path = os.path.join(self.scar_dir, self.images[index][:-4]+'sc_gt.npy')
        ...
        scar = np.load(scar_path,allow_pickle=True, fix_imports=True)
        scar[np.where(scar>0)]=1
        
        
        image=np.expand_dims(image, axis=0)
        mask=np.expand_dims(mask, axis=0)
        scar=np.expand_dims(scar, axis=0)
            
        return image,mask,scar,sp_dim,self.images[index]
    
def Data_Loader( test_dir,test_maskdir,sacr_dir,batch_size,num_workers=NUM_WORKERS,pin_memory=PIN_MEMORY):
    
    test_ids = Dataset_( image_dir=test_dir, mask_dir=test_maskdir,scar_dir=sacr_dir)
...
val_imgs="/home/akr54/from_data/imgs/"
val_masks="/home/akr54/from_data/LA/"
scar_masks="/home/akr54/from_data/Scars/"
...
test_loader=Data_Loader(val_imgs,val_masks,scar_masks,batch_size)
...
weights_paths="/home/akr54/patches/train4.pth.tar"

path_pre="/home/akr54/from_data/pre_pre/"
path_gt="/home/akr54/from_data/pre_gt/"
...
def check_accuracy(loader, model, device=DEVICE):
    ...
    model.eval()
    with torch.no_grad():
        for batch_idx, (data, t1,t2,sp_dim,label) in enumerate(loop):
            ...
            merged_gt=np.zeros([sp_dim,sp_dim])
            
            filename_p = os.path.join(path_pre,label[0])
            filename_g = os.path.join(path_gt,label[0])
            ...
            for x in range(wall.shape[0]):
                for y in range(wall.shape[1]):
                    if wall[y,x]==1.0:
                        ...
                        gt_crop=t2[y-32:y+32,x-32:x+32]
                        ...
                        #gt_crop=gt_crop.cpu()
                        #gt_crop=gt_crop.numpy()
                            
                        merged_pred[y-32:y+32,x-32:x+32]=  p2
                        merged_gt[y-32:y+32,x-32:x+32]=  gt_crop
             
               
            print((2 * (merged_pred * merged_gt).sum()) / (
                    (merged_pred + merged_gt).sum() + 1e-8
                ))
            
            np.save(filename_p, merged_pred)
            np.save(filename_g, merged_gt)
    ...
    #             (t1 + p1).sum() + 1e-8
    #         )
    # print(f"Dice score for Segmentation of scars: {dice_score1/len(test_loader)}")
