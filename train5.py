...
class Dataset_(Dataset):
    ...
    def __getitem__(self, index):
        ...
        gen_img=np.zeros([2,image.shape[0],image.shape[1]]) 
...
def save_checkpoint(state, filename="train5.pth.tar"):
...
def check_accuracy(loader, model, device=DEVICE):
    ...
    with torch.no_grad():
        for batch_idx, (img1,gt1,label) in enumerate(loop):
            ...
            torchvision.utils.save_image(img1[0,0,:,:],str(batch_idx)+'img0.png')
            torchvision.utils.save_image(img1[0,1,:,:],str(batch_idx)+'img1.png')
            torchvision.utils.save_image(gt1[0,0,:,:],str(batch_idx)+'gt.png')
            
            torchvision.utils.save_image(p1_[0,:,:,:],str(batch_idx)+'pre.png')
